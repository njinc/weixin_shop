...
class OrderAction extends userbaseAction {
	...
	public function closeOrder()//关闭订单
	 {
		...
		if (!is_array($order)) {
			...
			if ($item_order->where('orderId='.$orderId)->save($data)) {
				//设置为关闭
				$order_details = $order_detail->where('orderId='.$orderId)->select();
	...
	public function jiesuan() {
		...
		if (count($_SESSION['cart']) > 0) {
			$shop_address_mod = M('address');
			$shop_address_list = $shop_address_mod->select();
			$this->assign('shop_address_list', $shop_address_list);
